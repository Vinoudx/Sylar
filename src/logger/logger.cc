...
namespace sylar{
    ...
    Logger::Logger(LogLevel level, const std::string& name):name_(name),level_(level){
        appenders_.push_back(sylar::LogAppenderBase::ptr_t(new sylar::StdAppender()));
    }
    ...
    void StdAppender::log(LogLevel level, const LogEvent::ptr_t& event){
        AsyncLogger::getSingleton()->log(STDOUT_FILENO, formatter_->format(level, event));
    }
    ...
    void FileAppender::log(LogLevel level, const LogEvent::ptr_t& event){
        if(reopen()){
            AsyncLogger::getSingleton()->log(fd_, formatter_->format(level, event));
        }
    }
